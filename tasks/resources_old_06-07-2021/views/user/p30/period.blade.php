...
<div class="modal fade bs-example-modal-sm" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" style="margin-top:10%">
  <div class="modal-dialog modal-sm" role="document">
    <form action="<?php echo URL::to('user/add_p30_period'); ?>" method="post" class="addsp">
    ...
    <form action="<?php echo URL::to('user/update_p30_period'); ?>" method="post" class="editsp">
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        ...
        <tbody>
          ...
          <tr>            
            ...
            <td>
                <?php
                if($period->status ==0){
                  echo'<a href="'.URL::to('user/deactive_p30_period',base64_encode($period->id)).'"><i style="color:#00b348;" class="fa fa-check" aria-hidden="true"></i></a>';
                }
                else{
                  echo'<a href="'.URL::to('user/active_p30_period',base64_encode($period->id)).'"><i style="color:#f00;" class="fa fa-times" aria-hidden="true"></i></a>';
                }
                ?>
...
$(".editclass").click( function(){
  ...
  console.log(editid);
  $.ajax({
      url: "<?php echo URL::to('user/edit_p30_period') ?>"+"/"+editid,
      dataType:"json",
      type:"post",
...
$(window).change(function(e){
  if($(e.target).hasClass('sort_period'))
  {
    ...
    $.ajax({
      url:"<?php echo URL::to('user/period_sort_order'); ?>",
      type:"get",
