...
class TimejobController extends Controller {
	...
	*/
	...
	public function stop_job_details(){
		...
		$hour = $explode[0];
		$min = $explode[1];
		echo json_encode(array('id' => $job_details->id, 'start_time' =>date('H:i', strtotime($job_details->start_time)), 'start_hour' => $hour, 'start_min' => $min,'date'=>date('m-d-Y', strtotime($job_details->job_date))));
	}
	...
	public function jobuserfilter(){
		...
		$getdetails = DB::table('task_job')->where('user_id',$id)->where('job_date',$currentdate)->first();
		if(count($getdetails))
		{
			$todaystarttime = strtotime($currentdate.' '.$getdetails->start_time);
                  ...
                  if($jobs->status == 0){
                    ...
                    if($current_date != $jobs->job_date)
                    {
                  	...
                  	$output.='
				              ...
				              <td align="left" style="'.$redcolor.'">'.date('m-d-Y', strtotime($jobs->job_date)).'</td>
				              <td align="left" style="'.$redcolor.'">'.date('h:i A', strtotime($jobs->start_time)).'</td>
				              ...
				              <td align="center" style="'.$redcolor.'"><a style="'.$redcolor.'" href="javascript:" class="stop_class" data-element="'.$jobs->id.'">Stop</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a style="'.$redcolor.'" href="javascript:" class="create_new_quick" data-element="'.$jobs->id.'">Create a Quick Break</a></td>
				            </tr>
                  	';
                  	$i++;
                  }
      ...
      if($i == 1){
	    ...
	    $check_date_available = DB::table('task_job')->where('user_id',$id)->where('job_date',$curr_date)->where('quick_job',0)->where('status',0)->first();
      ...
      echo json_encode(array('activejob' => $output, 'closejob' => $outputclose,'username' => $username,'spendminutes' => $spendminutes,'spendhours' => $spendhours,'job_available' => $job_available));
	...
	public function searchjobofday(){
        ...
        <tr>
          ...
          <td><a href="javascript:"><i class="fa fa-pencil edit_task" data-element="'.$jobs->id.'" data-toggle="tooltip" title="Edit Job" aria-hidden="true"></i></a></td>
	...
	public function calculate_break_time()
	{
		...
		if($count_minues > $count_jon_time_minutes)
		{
			echo json_encode(array('alert' => 1));
		}
		else{
			if($count_minues == 0)
	        {
	        ...
	        echo json_encode(array('alert' => 0, 'break_hours' => $break_hours, 'count' => $count_minues));
		}
