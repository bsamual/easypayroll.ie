	...
	public function finance_reconcile_load(){
		...
		if(count($receipts_details)){
			foreach ($receipts_details as $receipts) {				
				...
				<td>'.$description.'</td>
				<td><a href="javascript:" class="receipt_class" data-element="'.$receipts->id.'">Receipts</a></td>
		...
		if(count($payment_details)){
			foreach ($payment_details as $payment) {
				...
				<td>'.$description.'</td>
				<td><a href="javascript:" class="payment_class" data-element="'.$payment->payments_id.'">Payments</a></td>
	...
	public function finance_bank_all_accept(){
		...
		if(count($payment_details)){
			foreach ($payment_details as $payment) {				
				...
				$output_payment.='<tr>
				<td>'.date('d-M-Y', strtotime($payment->payment_date)).'--'.$payment->payments_id.'</td>
				<td>'.$description.'</td>
				<td>Payments</td>
				<td align="right" id="payment_amount_'.$payment->payments_id.'">'.number_format_invoice($payment->amount).'</td>
				<td style="color:'.$color.'; text-align:right; font-weight:bold" id="payment_out_'.$payment->payments_id.'">'.number_format_invoice($outstanding).'</td>
	...
	public function finance_analysis_report(){
		...
		if($format == 3){
			...
			if($type == 1){
				...
				$output='<table class="table own_table_white" border="0px" cellpadding="0px" cellspacing="0px" style="font-family: Roboto, sans-serif; font-size: 13px; max-width: 600px; line-height: 20px; width: 100%; margin: 0px auto; ">
							<thead>
								<tr>
									...
									<td style="padding: 8px; border-bottom: 1px solid #c3c3c3; text-align:left;background:#000; color:#fff;'.$font_family.'">Company Name</td>
									<td style="widtd:120px; padding: 8px; border-bottom: 1px solid #c3c3c3; text-align:left; background:#000; color:#fff;'.$font_family.'">Date</td>
									<td style="widtd:230px; padding: 8px; border-bottom: 1px solid #c3c3c3; background:#000; color:#fff; text-align:left;'.$font_family.'">Source</td>
									<td style="padding: 8px; border-bottom: 1px solid #c3c3c3; background:#000; color:#fff; text-align:left;'.$font_family.'">Description</td>
									<td style="widtd:120px; text-align:right; padding: 8px; border-bottom: 1px solid #c3c3c3; background:#000; color:#fff;'.$font_family.'">Debit €</td>
									<td style="widtd:120px; text-align:right; padding: 8px; border-bottom: 1px solid #c3c3c3; background:#000; color:#fff;'.$font_family.'">Creedit €</td>
									<td style="widtd:120px; text-align:right; padding: 8px; border-bottom: 1px solid #c3c3c3; background:#000; color:#fff;'.$font_family.'">Balance</td>
								</tr>
								</thead><tbody>';
				$date = DB::table('user_login')->where('id',1)->first();
				if(count($clientlist)){
					foreach ($clientlist as $client) {
						...
						if(count($finance_client)){
							...
							$client_opening = '<tr>
										<td style="padding: 8px; border-bottom: 1px solid #c3c3c3; text-align:left;'.$font_family.'">'.$client_details->client_id.'</td>
										<td style="padding: 8px; border-bottom: 1px solid #c3c3c3; text-align:left;'.$font_family.'">'.$companyname.'</td>
						...
						if(count($get_receipt_payments))
						{
							...
							foreach($get_receipt_payments as $list)
							{
								...
								$client_details = DB::table('cm_clients')->where('client_id', $list->client_code)->first();
			    ...
			    $pdf->save('papers/Client Finance Account.pdf');

			    $filename_download = 'Client Finance Account.pdf';
		...
		if($format == 4){
			...
			if($type == 1){
				$columns = array('Client Code','Company Name','Date','Source','Description', 'Debit', 'Credit','Balance');
				fputcsv($file, $columns);
				...
				if(count($clientlist)){
					foreach ($clientlist as $client) {
						...
						if(count($finance_client)){
							...
							$columns = array($client_details->client_id,$companyname,date('d-M-Y', strtotime($date->opening_balance_date)),'Opening Balance',$owed_text, $openening_debit, $openening_credit,number_format_invoice($finance_client->balance));
							fputcsv($file, $columns);
						}
						else{
							$balance='';
						...
						if(count($get_receipt_payments))
						{
							...
							foreach($get_receipt_payments as $list)
							{
								...
								$columns = array($client_details->client_id,$companyname,date('d-M-Y', $list->dateval),$source,$textvalue, $amount_debit, $amount_credit,number_format_invoice($balance_val));
								fputcsv($file, $columns);
							}
						}
					}					
