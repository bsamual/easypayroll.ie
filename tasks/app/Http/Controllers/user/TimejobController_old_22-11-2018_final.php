...
class TimejobController extends Controller {
	...
	*/
	...
	public function stop_job_details(){
		...
		$hour = $explode[0];
		$min = $explode[1];
		echo json_encode(array('id' => $job_details->id, 'quick_job_times' => $quick_job_times, 'start_time' =>date('H:i', strtotime($job_details->start_time)), 'start_hour' => $hour, 'start_min' => $min,'date'=>date('d-M-Y', strtotime($job_details->job_date))));
	}
	...
	public function jobuserfilter(){
      ...
      if($i == 1){
	    ...
	    if($id != "")
	    {
		    ...
		    $getdetails = DB::table('task_job')->where('user_id',$id)->where('job_date',$currentdate)->get();
		    $spendminutes = 0;
        	$spendhours = 0;
			if(count($getdetails))
			{
			...
			foreach ($job_details as $single) {
				...
				if(count($job_details_child)) {
					foreach ($job_details_child as $child) {
				        ...
				        $jobtime =   $hours.':'.$minutes.':'.$second;
				      	$columns_2 = array($i.'.'.$ichild, $user_details->firstname, $companyname, $task_details->task_name, $task_type, date('d-M-Y', strtotime($child->job_date)), date('H:i:s', strtotime($child->start_time)), $jobtime );
						fputcsv($file, $columns_2);
						$ichild++;
					}
				}
	...
	public function active_jobreportpdf(){
        ...
        <tr>          
          ...
          <span id="job_time_refresh_'.$child->id.'">'.$jobtime.'</span> &nbsp;&nbsp;<a href="javascript:"><i class="fa fa-refresh job_time_refresh" aria-hidden="true" data-element="'.$child->id.'"></i></a>
	...
	public function all_job_report_csv(){
		...
		$callback = function() use ($job_details, $columns)
    	{
			...
			foreach ($job_details_child as $child) {				
                ...
                else if($jobs->quick_job == 1 && $jobs->status == 1){
                  $job_time_checked = $jobs->job_time;
                }
                else if($jobs->quick_job == 0 && $jobs->status == 1){
                  $job_time_checked = $jobs->job_time;
	...
	public function all_jobreportpdf(){
        ...
        if(count($joblist_child)){              
          foreach ($joblist_child as $child) {
                ...
                else if($child->quick_job == 1 && $child->status == 1){
	...
	public function calculate_break_time()
	{
		...
		if($count_minues > $count_jon_time_minutes)
		{
			...
			{
				$alert = 1;
				$total_time_minutes_format = '00:00:00';
			}
			else{
