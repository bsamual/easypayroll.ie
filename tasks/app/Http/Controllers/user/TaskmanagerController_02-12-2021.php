...
class TaskmanagerController extends Controller {
	...
	*/
	...
	public function task_overview()
	{
		...
		return view('user/task_manager/task_overview', array('title' => 'Easypayroll - Task Overview','user_tasks' => $user_tasks,'open_task_count' => $open_task_count,'authored_task_count' => $authored_task_count,'park_task_count' => $park_task_count, 'userlist' => $user,'clientlist' => $clientlist));
	}
	...
	public function taskmanager_change_due_date()
	{
	    ...
	    if($task_details->avoid_email == "0")
	    {
			...
			if(($task_details->author_email == "") || ($task_details->author_email == $author_email)) {
				$aut_email = 'info@gbsco.ie';
			}
			else{
				$aut_email  = $task_details->author_email;
			}
	...
	public function taskmanager_change_allocations()
	{
		...
		if($task_details->avoid_email == "0")
	    {
			...
			if(($task_details->author_email == "") || ($task_details->author_email == $allocated_email)) {
				$aut_email = 'info@gbsco.ie';
			}
			else{
				$aut_email  = $task_details->author_email;
	...
	public function show_existing_comments()
	{
		...
		echo json_encode(array("output" => $output, "auto_close" => $task_details->auto_close, "show_auto_close_msg" => $show_auto_close_msg, 'title' => 'Title: '.$subject, "task_specifics_name" => $task_specifics_name));
	}
