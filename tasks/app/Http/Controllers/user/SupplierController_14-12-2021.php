	...
	public function get_supplier_transaction_list()
	{
		...
		if(count($details))
		{
			...
			elseif($opening_bal > 0)
			{
				...
				$payment_opening='';
				$invoice_opening='Opening Balance';
			}
			else{
				$color = 'red';
			...
			<div class="'.$class.'" style="max-height:700px; overflow:hidden; overflow-y:scroll;">
				...
				<table class="table own_table_white client_account_table" style="width: 100%;">
				<thead>
				<tr>
					<th>Date</th>
				...
				<tbody id="client_account_tbody">
					<tr>
						...
						<td style="color:'.$color.'">'.$payment_opening.'</td>
						<td style="color:'.$color.'">'.$invoice_opening.'</td>
						<td></td>
						<td></td>
					...
					if(count($get_invoice_payments))
					{
						foreach($get_invoice_payments as $list)
						{
							if(isset($list->invoice_no)) { 
								...
								$invocie_text = '<a href="javascript:" class="purchase_class" style="'.$colorval.'" data-element="'.$list->id.'">Purchase Invoice</a>';
								$payment_text = '';
								...
								$invocie_text = '';
								$payment_text = '<a href="javascript:" class="payment_class" style="'.$colorval.'" data-element="'.$list->payments_id.'">Payments</a>';
								$download='';
							}
							...
							$output.='<tr>
								...
								<td style="'.$colorval.'">'.$payment_text.'</td>
								<td style="'.$colorval.'">'.$invocie_text.'</td>
								<td style="'.$colorval.'">'.$textvalue.'</td>
								<td style="'.$colorval.'" align="center">'.$download.'</td>
								<td style="'.$colorval.'" align="right">'.$amount.'</td>
