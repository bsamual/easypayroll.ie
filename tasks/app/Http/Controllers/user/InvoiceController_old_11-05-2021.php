	...
	*/
	...
	public function insert_update_invoice_nominals()
	{
		...
		if(count($check_invoice))
		{
			...
			DB::table('invoice_journals')->where($check_invoice->reference)->where('nominal_code','001')->update($data);

			$data['dr_value'] = '0.00';
			$data['cr_value'] = number_format_invoice_without_comma($invoice_details->vat_value);
			DB::table('invoice_journals')->where($check_invoice->reference)->where('nominal_code','845')->update($data);

			$data['dr_value'] = number_format_invoice_without_comma($invoice_details->gross);
			$data['cr_value'] = '0.00';
			DB::table('invoice_journals')->where($check_invoice->reference)->where('nominal_code','712')->update($data);
