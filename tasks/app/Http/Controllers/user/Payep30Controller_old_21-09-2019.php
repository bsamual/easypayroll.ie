...
class Payep30Controller extends Controller {
	...
	*/
	...
	public function update_paye_p30_year_disabled_status()
	{
		...
		if(count($tasks))
		{
			foreach($tasks as $task)
			{
				...
				$check_periods = DB::table('paye_p30_periods')->where('paye_task',$task->id)->where('month_id',$active_month)->where('last_email_sent','!=', '0000-00-00 00:00:00')->count();
				if($check_periods > 0)
				{
					if($task_array == "")
					{
	...
	public function paye_p30_create_csv()
	{
		...
		$get_periods = DB::table('paye_p30_periods')->where('paye_task',$task_id)->get();
		...
		if(count($get_details))
		{
			...
			$columns3 = array('','Ros Liability','Task Liability','Diff','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','','');

			fputcsv($file, $columns3);

			if(count($get_periods))
			{
				foreach($get_periods as $period)
				{
					if($period->month_id == 1) { $month_name = 'Jan - '.$get_year->year_name; }
					if($period->month_id == 2) { $month_name = 'Feb - '.$get_year->year_name; }
					if($period->month_id == 3) { $month_name = 'Mar - '.$get_year->year_name; }
					if($period->month_id == 4) { $month_name = 'Apr - '.$get_year->year_name; }
					if($period->month_id == 5) { $month_name = 'May - '.$get_year->year_name; }
					if($period->month_id == 6) { $month_name = 'June - '.$get_year->year_name; }
					if($period->month_id == 7) { $month_name = 'July - '.$get_year->year_name; }
					if($period->month_id == 8) { $month_name = 'Aug - '.$get_year->year_name; }
					if($period->month_id == 9) { $month_name = 'Sep - '.$get_year->year_name; }
					if($period->month_id == 10) { $month_name = 'Oct - '.$get_year->year_name; }
					if($period->month_id == 11) { $month_name = 'Nov - '.$get_year->year_name; }
					if($period->month_id == 12) { $month_name = 'Dec - '.$get_year->year_name; }
	...
	public function check_paye_task_details()
	{
		...
		if($show_option == 1)
		{
			...
			$output = '<div class="row">
				<div class="col-md-6">
					<div class="col-md-2">
						<input type="radio" name="update_task_radio" class="update_task_radio update_task_week" value="'.$task_id_week.'" id="update_task_radio'.$task_id_week.'">
				...
				<div class="col-md-6">
					<div class="col-md-2">
						<input type="radio" name="update_task_radio" class="update_task_radio update_task_month" value="'.$task_id_month.'" id="update_task_radio'.$task_id_month.'">
		...
		echo json_encode(array("type" => $type,"output" =>$output,'outputtext' => $outputtext));
	}
	...
	public function paye_p30_payment_update(){
		...
		DB::table('paye_p30_periods')->where('period_id', $id)->update(['payments' => $payment_value]);
