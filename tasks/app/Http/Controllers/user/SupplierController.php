...
class SupplierController extends Controller {
	...
	*/
	...
	public function store_supplier()
	{
		...
		$data['default_nominal'] = Input::get('default_nominal');
		$data['username'] = Input::get('ac_username');
		$data['password'] = base64_encode(Input::get('ac_password'));		
	...
	public function edit_supplier()
	{
		...
		{
			$journals_details = DB::table('journals')->where('connecting_journal_reference', $get_details->journal_id)->first();

			if(count($journals_details)){
	...
	public function refresh_all_supplier_counts(){
		$suppliers = DB::table('suppliers')->get();
		$dataval = array();
		if(count($suppliers)) {
	...
	public function supplier_journal_create(){
		$id = base64_decode(Input::get('id'));

		$suppliers_details = DB::table('suppliers')->where('id', $id)->first();
		$finance_date = DB::table('user_login')->where('id',1)->first();
		$journals = DB::table('journals')->groupBy('reference')->get();		

		$journal_id = count($journals)+1;
		
		if(count($suppliers_details)){
			...
			if($suppliers_details->opening_balance > 0){
				...
				DB::table('journals')->insert($data_secondary);
		...
		DB::table('suppliers')->where('id', $id)->update(['journal_id' =>  $journal_id]);

		$href='<a href="javascript:" class="journal_id_viewer" data-element="'.$journal_id.'">'.$journal_id.'</a>';

		echo json_encode(array('id' => $href));
	}
	public function run_creditors_listing(){
		$nil = Input::get('removed');
		$dateval = explode('/',Input::get('dateval'));
		...
		if(count($suppliers))
		{
			foreach($suppliers as $supplier){
				...
				$invoice_count = DB::table('supplier_global_invoice')->select(DB::raw('count(*) as invoice_count, sum(gross) as gross_sum'))->where('supplier_id',$supplier->id)->where('invoice_date','<=',$datevall)->first();

	            $payment_sum = DB::table('payments')->select(DB::raw('sum(amount) as payment_sum'))->where('imported',0)->where('debit_nominal','813')->where('supplier_code',$supplier->id)->where('payment_date','<=',$datevall)->first();

	            $balance = ($opening_balance + $invoice_count->gross_sum) - $payment_sum->payment_sum;
	...
	public function export_creditors_listing(){
		$nil = Input::get('removed');
		$dateval = explode('/',Input::get('dateval'));
		$datevall = $dateval[2].'-'.$dateval[1].'-'.$dateval[0];
