							...
							if($act == 0){
								...
								if(count($setting_name))
								{
									if(count($get_attachments)) { 
										...
										$documents.='<p><input type="checkbox" name="year_end_documents" class="year_end_documents" id="year_end_documents_'.$year->year.'_'.$key.'" data-element="'.$attach_id.'"><label for="year_end_documents_'.$year->year.'_'.$key.'">'.$setting_name->document.' ('.$attachment.')</label></p>';
									}
									else{ 
										$attachment = '<spam style="color:#f00">No Document</spam>'; 
										$attach_id = 0; 

										$documents.='<p><input type="checkbox" name="year_end_documents" class="year_end_documents" id="year_end_documents_'.$year->year.'_'.$key.'" data-element="'.$attach_id.'" disabled><label for="year_end_documents_'.$year->year.'_'.$key.'">'.$setting_name->document.' ('.$attachment.')</label></p>';
									}
	...
	public function upload_key_docs_letter(){
		...
		if (!empty($_FILES)) {
			...
			elseif($type == "3"){
		 	...
		 	echo json_encode(array('id' => $insertedid,'filename' => $fname,'client_id' => $client_id, 'output' => $output,'type' => $type));
		}
	}
