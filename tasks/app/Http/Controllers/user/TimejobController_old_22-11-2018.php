...
class TimejobController extends Controller {
	...
	*/
	...
	public function stop_job_details(){
		...
		$hour = $explode[0];
		$min = $explode[1];
		echo json_encode(array('id' => $job_details->id, 'start_time' =>date('H:i', strtotime($job_details->start_time)), 'start_hour' => $hour, 'start_min' => $min,'date'=>date('d-M-Y', strtotime($job_details->job_date))));
	}
                  ...
                  if($jobs->status == 0){
                    ...
                    if($current_date != $jobs->job_date)
                    {
                  	...
                  	$output.='
				              ...
				              <td align="center" style="'.$redcolor.'"><a style="'.$redcolor.'" href="javascript:" class="stop_class" data-element="'.$jobs->id.'">Stop</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a style="'.$redcolor.'" href="javascript:" class="create_new_quick" data-element="'.$jobs->id.'">Quick Job</a></td>
				            </tr>
                  	';
                  	$i++;
                  }
