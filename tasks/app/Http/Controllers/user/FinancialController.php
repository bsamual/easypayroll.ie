...
class FinancialController extends Controller {
	...
	*/
	...
	public function finance_reconcile_load(){
		...
		if(count($receipts_details)){
			foreach ($receipts_details as $receipts) {				
				...
				$code_description = DB::table('nominal_codes')->where('code',$receipts->credit_nominal)->first();
				$description = $receipts->credit_nominal.'-'.$receipts->credit_description;
				if(count($code_description)){
		...
		if(count($payment_details)){
			foreach ($payment_details as $payment) {
				...
				$code_description = DB::table('nominal_codes')->where('code',$payment->debit_nominal)->first();
	...
	public function generate_reconcile_pdf() {
		...
		if(count($receipts)) {
			foreach($receipts as $receipt){
				$code_description = DB::table('nominal_codes')->where('code',$receipt->credit_nominal)->first();
	...
	public function practice_load_review(){
		$year = Input::get('year');
        ...
        for($i = 1; $i <= 12; $i++){
          ...
          $net = DB::table('invoice_system')->where('invoice_date','>=',$from_date)->where('invoice_date','<=',$to_date)->sum('inv_net');
	...
	public function practice_load_client_review(){
		$count = Input::get('count');
		$redact = Input::get('redact');

		$client_details = DB::table('cm_clients')->where('id', $count)->first();
        ...
        for($i = $first_year; $i <= $last_year; $i++){
          ...
          $net = DB::table('invoice_system')->where('client_id', $client_details->client_id)->where('invoice_date','>=',$from_date)->where('invoice_date','<=',$to_date)->sum('inv_net');
		...
		echo json_encode(array('output' => $result, 'client_id' =>$client_details->client_id, 'count' => $count ));

		
	}

	public function practice_load_staff_review(){
		$userlist = DB::table('user')->where('disabled',0)->get();
	...
	public function practice_review_export(){
		$type = Input::get('type');
		...
		elseif($type == 2){
		    ...
		    $client_list = DB::table('cm_clients')->get();
		    ...
		    if(count($client_list)){
		    	foreach ($client_list as $client) {
			        ...
			        for($i = $first_year; $i <= $last_year; $i++){
			          ...
			          $net = DB::table('invoice_system')->where('client_id', $client->client_id)->where('invoice_date','>=',$from_date)->where('invoice_date','<=',$to_date)->sum('inv_net');
