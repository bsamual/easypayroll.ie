	...
	*/
	...
	public function yearend_crypt_validdation(){
      ...
      public function yearend_create_new_year()
      {
            ...
            $client_details = DB::table('cm_clients')->where('active', 1)->orderBy('id','ASC')->get();

            if(count($client_details)){
                  foreach ($client_details as $client) {
                        $get_year_client = DB::table('year_client')->where('client_id',$client->client_id)->where('year',$get_last_created_year->year)->first();
                        $status = 0;
                        if(count($get_year_client)){
      ...
      public function save_yearend_date_status()
      {
      ...
      public function save_yearend_liability_date()
      {
            $dateval = Input::get('dateval');
            $client_id = Input::get('client_id');
            $setting_id = Input::get('setting_id');
            $year_id = Input::get('year_id');
            $yearend_client = DB::table('year_client')->where('year', $year_id)->where('client_id', $client_id)->first();
