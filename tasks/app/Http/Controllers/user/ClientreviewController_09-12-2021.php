...
class ClientreviewController extends Controller {
	...
	*/
	...
	public function client_review_load_all_client_receipt()
	{
		...
		{
			$year = Input::get('year');
			$receiptlist = DB::select('SELECT * from `receipts` WHERE client_code LIKE "%'.$client_id.'%" AND imported = "0" AND `receipt_date` LIKE "'.$year.'%"');
			$margin_top = 'margin-top:48px !important;';
		}
		elseif($type == "2")
		{
			$receiptlist = DB::table('receipts')->where('client_code', 'like', '%'.$client_details->client_id.'%')->where('imported',0)->get();
		...
		elseif($type == "3")
		{
			...
			$receiptlist = DB::table('receipts')->where('client_code','like', '%'.$client_details->client_id.'%')->where('receipt_date','>=',$from)->where('receipt_date','<=',$to)->where('imported',0)->get();
