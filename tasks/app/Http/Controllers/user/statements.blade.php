...
#own_table_white2 tr:nth-child(2n) td:first-child
{
...
function next_client_bal_check(count)
{
  if(count == 0)
  {
  ...
  $.ajax({
    ...
    {
      $(".client_tr_"+client_id).find(".opening_bal_td").html(result);
      setTimeout( function() {
        var countval = count + 1;
        if($(".client_tr:eq("+countval+")").length > 0)
        {
          next_client_bal_check(countval);
