	...
	*/
	...
	public function taskmanager_change_due_date()
	{
	    ...
	    if($task_details->avoid_email == "0")
	    {
			...
			$email = new PHPMailer();
			$email->SetFrom($user_details->email);
			$email->Subject   = $subject_email;
			$email->Body      = $contentmessage;
			if($task_details->allocated_to != 0)
	    	{
	...
	public function show_existing_comments()
	{
		...
		echo json_encode(array("output" => $output, "auto_close" => $task_details->auto_close, "show_auto_close_msg" => $show_auto_close_msg, 'title' => 'Title: '.$subject, "task_specifics_name" => $task_specifics_name, "two_bill" => $task_details->two_bill));
	}
	...
	public function request_update()
	{
		...
		if($task_details->allocated_to != 0)
    	{
			...
			$email = new PHPMailer();
			$email->SetFrom($author_details->email);
			$email->Subject   = $subject_email;
			$email->Body      = $contentmessage;
			$email->AddCC('tasks@gbsco.ie');
			$email->IsHTML(true);
