...
class Payep30Controller extends Controller {
	...
	*/
	...
	public function paye_p30month_manage($id)
	{
		$id = base64_decode($id);
		$year = DB::table('paye_p30_year')->where('year_id',$id)->first();
		$month = DB::table('paye_p30_month')->where('year', $id)->orderBy('month_id','dec')->get();
		return view('user/paye_p30/paye_p30_month_manage', array('monthlist' => $month,'year' => $year));
	}
	...
	public function paye_p30_email_unsent_files()
	{
		...
		if(count($explode))
		{
			...
			return redirect('user/paye_p30_select_month/'.$encoded_month_id.'?divid=taskidtr_'.$task_id)->with('message', 'Email Sent Successfully');
		}
		else{
			return redirect('user/paye_p30_select_month/'.$encoded_month_id.'?divid=taskidtr_'.$task_id)->with('error', 'Email Field is empty so email is not sent');
		}
	}
}
