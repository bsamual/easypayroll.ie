...
class InfileController extends Controller {
	...
	*/
	...
	public function create_new_file()
	{
		...
		return redirect('user/in_file')->with('message', 'InFiles for "'.$client_details->firstname.'&nbsp;'.$client_details->surname.'" is Created Successfully and saved the Selection.');
	}
	...
	public function report_infile(){
		$id = Input::get('id');
		if($id == 0){
				$infilelist = DB::table('in_file')->groupBy('client_id')->get();				
				$output = '';
				$i=1;
				if(count($infilelist)){ 
	...
	public function infile_report_csv_single($id=""){		
		...
		$columns_1 = array('#', 'Client Name', 'Date Received', 'Date Added', 'No. of Files', 'Files List');
		...
		$callback = function() use ($infile_client, $columns_1)
    	{
			...
			foreach ($infile_client as $singlefile) {
				...
				if(count($first_file_name))
				{
		      	...
		      	$columns_2 = array($i, $client_details->firstname, $singlefile->data_received, $singlefile->date_added, $file_count, $first_file);
				fputcsv($file, $columns_2);
			...
			foreach ($infile_client as $file) {
				...
				$output.='
					<tr>
						...
						<td style="text-align: left;border:1px solid #000;">'.$file->date_added.'</td>						
						<td style="text-align: left;border:1px solid #000;">'.$file_count.'</td>
						<td style="text-align: left;border:1px solid #000;">'.$infile_attached.'</td>
