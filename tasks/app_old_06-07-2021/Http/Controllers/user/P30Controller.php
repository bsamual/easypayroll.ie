...
class P30Controller extends Controller {
	...
	*/
	...
	public function p30()
	{
		...
		return view('user/p30/month', array('title' => 'Select Year', 'yearlist' => $year, 'paye_p30_year' => $paye_p30_year));
	}
	public function p30monthmanage($id=""){
		$id = base64_decode($id);
		$year = DB::table('year')->where('year_id',$id)->first();
		$month = DB::table('p30_month')->where('year', $id)->orderBy('month_id','dec')->get();
		return view('user/p30/month_manage', array('monthlist' => $month,'year' => $year));
	}
	public function p30selectmonth($id="")
	{
		$id =base64_decode($id);
		...
		return view('user/p30/select_month', array('title' => 'P30 Month Task', 'yearname' => $year_id, 'monthid' => $month_id, 'resultlist' => $result_task));
	}
	public function review_month($id="")
	{
		...
		return redirect('user/p30_select_month/'.base64_encode($id))->with('message', 'Reviewed Successfully.');
	}
	public function close_create_new_month($id="")
	{
		...
		{
			return redirect('user/p30_select_month/'.base64_encode($month_id->month_id))->with('message', 'Already 12 months has been created in this year. So Please contact admin to create new year.');
		}
		else
		{
		...
		return redirect('user/p30_select_month/'.base64_encode($new_id))->with('message', 'Month created Successfully.');
	}
	public function p30tasklevelupdate($id=""){
		$id = Input::get('id');		
		$value = Input::get('value');
		
		DB::table('p30_task')->where('id', $id)->update(['task_level' => $value]);
	}
	public function p30periodupdate($id=""){
		$id = Input::get('id');		
		$value = Input::get('value');
		DB::table('p30_task')->where('id', $id)->update(['task_period' => $value]);
	}
	public function payp30()
	{
		$id = Input::get('id');
		$pay = Input::get('pay');
		DB::table('p30_task')->where('id',$id)->update(['pay' => $pay]);
	}
	public function nap30()
	{
		$id = Input::get('id');
		$pay = Input::get('na');
		DB::table('p30_task')->where('id',$id)->update(['na' => $pay]);
	}
	public function emailp30()
	{
		$id = Input::get('id');
		$email = Input::get('email');
		DB::table('p30_task')->where('id',$id)->update(['email' => $email]);
	}
	public function p30_task_image_upload()
	{
		...
		for($i=0; $i<$total; $i++) {
			...
			$upload_dir = 'uploads/p30_task_image';
		...
		return redirect('user/p30_select_month/'.base64_encode($data_img->task_month).'?divid=taskidtr_'.$id);
	}
	public function p30_task_automatic_image_upload()
	{
		$id = Input::get('hidden_id');
		...
		$upload_dir = 'uploads/p30_task_xml';
		if (!file_exists($upload_dir)) {
			mkdir($upload_dir);
		}
		$upload_dir = $upload_dir.'/'.base64_encode($data_img->id);
	...
	public function p30_task_delete_image()
	{
		$imgid = Input::get('imgid');
		DB::table('p30_task_attached')->where('id',$imgid)->delete();
	}
	public function p30_task_delete_xml()
	{
		$imgid = Input::get('imgid');
	...
	public function p30_edit_email_unsent_files()
	{
		$task_id = Input::get('task_id');
		...
		$contentmessage = view('user/p30_email_content', $data)->render();
	...
	public function p30_email_unsent_files()
	{
		...
		if(count($attachments))
		{
			if(count($explode))
			{
				...
				return redirect('user/p30_select_month/'.$encoded_month_id.'?divid=taskidtr_'.$task_id)->with('message', 'Email Sent Successfully');
			}
			else{
				return redirect('user/p30_select_month/'.$encoded_month_id.'?divid=taskidtr_'.$task_id)->with('error', 'Email Field is empty so email is not sent');
			}
		}
		else{
			return redirect('user/p30_select_month/'.$encoded_month_id.'?divid=taskidtr_'.$task_id)->with('error', 'Attachments are empty so Email is not sent');
		}
	}
	public function p30_task_status_update()
	{
		$id = Input::get('id');
	...
	public function p30_report_task($id=""){
	...
	public function download_p30_pdf_report()
	{
	...
	public function import_p30_review_due()
	{
		...
		if($_FILES['import_csv']['name']!='')
		{
			...
			if(move_uploaded_file($tmp_name, "$uploads_dir/$name")){
				...
				foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
					...
					{
						return redirect('user/p30_select_month/'.base64_encode($monthid))->with('error_import', 'You have tried to upload a wrong csv file.');
					}
					else{
				...
				Session::put($sessn);
				return redirect('user/p30_select_month/'.base64_encode($monthid))->with('message_import', 'Reviewed successfully.');
			}
		}
	}
	public function download_p30_review()
	{
		$type = Input::get('type');
	...
	public function update_p30_incomplete_status_month()
	{
		$data['p30_incomplete'] = Input::get('value');
		DB::table('user_login')->where('userid',1)->update($data);
	}
	public function update_p30_na_status_month()
	{
		$data['p30_na'] = Input::get('value');
		DB::table('user_login')->where('userid',1)->update($data);
	}
}
