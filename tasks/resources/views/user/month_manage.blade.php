...
<style>
body{background:url('<?php echo URL::to('assets/images/bg_monthly.jpg')?>') no-repeat center top !important; 
        ...
        <table class="table table_bg table_style">
          ...
          <tbody>
            ...
            if(count($monthlist)){
              ...
              foreach($monthlist as $month){
                $task_count = DB::table('task')->where('task_month',$month->month_id)->count();
                $task_completed = DB::table('task')->where('task_month',$month->month_id)->where('task_status',1)->count();
                $task_donot_completed = DB::table('task')->where('task_month',$month->month_id)->where('task_status',0)->where('task_complete_period',1)->count();
                $task_incomplete = DB::table('task')->where('task_month',$month->month_id)->where('task_status',0)->where('task_complete_period',0)->count();
            ?>
