...
<style>
  ...
  .start_red { color:#ff0; }
  .start_lred { color:#ffda00; }
  .start_orange { color:#ec5300; }
  .start_brown { color:#ec2400; }
  .start_yellow { color:#ec0000; }
          ...
          <tbody id="task_body_std">
              ...
              <?php if($result->task_status == 1) { $task_label='style="color:#f00;font-weight:800"'; } elseif($result->task_complete_period == 1) { $task_label='style="color:#1b0fd4;font-weight:800"'; }  elseif($result->task_started == 1) { $task_label='style="color:#89ff00;font-weight:800"'; } else{ $task_label='style="color:#fff;font-weight:600"'; }  ?>
                  ...
                  <td>
                    ...
                    ?>
                    <input type="button" class="common_black_button secret_button" value="&nbsp;" style="width:60px;background: #ffa12d !important;margin-top:63px" data-element="<?php echo $result->task_id; ?>">
                  </td>
                  ...
                  <td align="center" class="special_td" style="width:90px;">
                    ...
                    <div class="special_div" style="text-align: left;border-top:1px solid #000;width:550px !important;clear:both;float:left;position: absolute;">
                      ...
                      ?>
                      <input type="checkbox" name="same_as_last" class="same_as_last" data-element="<?php echo $result->task_id; ?>" id="same_as_last_<?php echo $result->task_id; ?>" <?php echo $prev_checked; ?>><label class="same_as_last_label" for="same_as_last_<?php echo $result->task_id; ?>" style="<?php echo $prev_text; ?>">Same as last month</label>
...
$(window).click(function(e) {
  ...
  if($(e.target).hasClass('same_as_last'))
  {
    if($(e.target).is(":checked"))
    {
      ...
      {
        $.ajax({
          url:"<?php echo URL::to('user/check_previous_month'); ?>",
          type:"post",
          data:{task_id:task_id,month:"<?php echo $monthid->month_id; ?>",status:"1"},
          success:function(result)
          {
      ...
      $.ajax({
        url:"<?php echo URL::to('user/check_previous_month'); ?>",
        type:"post",
        data:{task_id:task_id,month:"<?php echo $monthid->month_id; ?>",status:"0"},
        success:function(result)
        {
          $(e.target).parents(".special_div").find(".files_received_div").html(result);
  ...
  if($(e.target).hasClass("create_task_manager"))
  {
    ...
    $.ajax({
      ...
      success: function(result)
      {
        ...
        $(".subject_class").val("Current Period (Monthly) Payroll to Be Created");
        ...
        alert('You must CHECK all the boxes of this task to Mark this task as Complete');
          ...
          if (r == true) {
              ...
              $.ajax({
                  ...
                  success: function(result) {
                      window.location.replace("<?php echo URL::to('user/select_month/'.base64_encode($monthid->month_id).'?divid='); ?>"+id);
