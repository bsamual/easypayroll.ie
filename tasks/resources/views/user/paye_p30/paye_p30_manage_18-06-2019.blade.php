...
<div id="alert_modal" class="modal fade" role="dialog" data-keyboard="false" data-backdrop="static" style="margin-top:100px">
  ...
  if(count($payelist)){
    foreach ($payelist as $keytask => $task) {
        ...
        $output.='<li '.$background.'>
                <div class="sno">'.$i.'</div>
                <div class="clientname">'.$task->task_name.' <a href="javascript:" class="load_info" data-element="'.$task->id.'"> Show Table </a>
  ...
  $.ajax({
    ...
    success: function(result)
    {
      ...
      if(result['show_active'] == 1)
      {
        ...
        var weekcountval = weekcount + 1;
        var monthcountval = monthcount + 1;

        var totalval = weekcountval + monthcountval;

        if(totalval > 60)
        {
          $(".table_paye_p30").css("width","6700px");
        }
        else if(totalval > 50)
        {
          $(".table_paye_p30").css("width","5700px");
        }
        else if(totalval > 40)
        {
          $(".table_paye_p30").css("width","5000px");
        }
        else if(totalval > 30)
        {
          $(".table_paye_p30").css("width","4000px");
        }
        else if(totalval > 20)
        {
          $(".table_paye_p30").css("width","3000px");
        }
        else if(totalval > 10)
        {
          $(".table_paye_p30").css("width","2000px");
        }
        else
        {
          $(".table_paye_p30").css("width","1000px");
        }
...
{
    $(e.target).parents(".clientname").find(".load_info_table").html("");
    $(e.target).parents(".clientname").find(".load_info_table").hide();
