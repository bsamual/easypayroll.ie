...
<style>
.flag_gray{ color:gray; }
.flag_orange{ color:orange; }
.flag_red{ color:red; }
          ...
          if(count($infiles)){
            foreach ($infiles as $file) {
              ...
              if(count($attachments)){  
                ...
                if($attachment->flag == 0) { $flag = '<i class="fa fa-flag flag_gray fileattachment file_attach_bpso" style="cursor:pointer" data-element="'.URL::to('/').'/'.$attachment->url.'/'.$attachment->attachment.'" '.$color.' data-toggle="tooltip" title="'.$attachment->attachment.'" data-src="'.$attachment->url.'/'.$attachment->attachment.'"></i>'; }
                elseif($attachment->flag == 1) { $flag = '<i class="fa fa-flag flag_orange flag_gray fileattachment file_attach_bpso" style="cursor:pointer" data-element="'.URL::to('/').'/'.$attachment->url.'/'.$attachment->attachment.'" '.$color.' data-toggle="tooltip" title="'.$attachment->attachment.'" data-src="'.$attachment->url.'/'.$attachment->attachment.'"></i>'; }
                elseif($attachment->flag == 2) { $flag = '<i class="fa fa-flag flag_red fileattachment file_attach_bpso" style="cursor:pointer" data-element="'.URL::to('/').'/'.$attachment->url.'/'.$attachment->attachment.'" '.$color.' data-toggle="tooltip" title="'.$attachment->attachment.'" data-src="'.$attachment->url.'/'.$attachment->attachment.'"></i>'; }
                        ...
                        <a href="javascript:" class="trash_icon '.$disable_class.'"><i class="fa fa-trash trash_image" data-element="'.$attachment->id.'" aria-hidden="true"></i></a>
											<span style="font-weight:800; font-size:15px;">'.substr($attachment->attachment,0,15).'</span>
...
$(window).click(function(e) {
 ...
 //  }
  ...
  if($(e.target).hasClass('fileattachment'))
  {
      ...
      if($(e.target).parents(".file_attachment_div").find(".fa-flag").hasClass('flag_gray'))
      {
        var flag_status = '1';
        $.ajax({
          ...
          success:function(result)
          {
            $(e.target).parents(".file_attachment_div").find(".fa-flag").removeClass('flag_gray');
            $(e.target).parents(".file_attachment_div").find(".fa-flag").addClass('flag_orange');
      ...
      else if($(e.target).parents(".file_attachment_div").find(".fa-flag").hasClass('flag_orange'))
      {
        var flag_status = '2';
        $.ajax({
          url:"<?php echo URL::to('user/change_flag_status'); ?>",
          ...
          success:function(result)
          {
            $(e.target).parents(".file_attachment_div").find(".fa-flag").removeClass('flag_orange');
            $(e.target).parents(".file_attachment_div").find(".fa-flag").addClass('flag_red');
            var element = $(e.target).attr('data-element');
            var src = $(e.target).attr('data-src');
            var srcc = $(e.target).attr('data-src');
            $('body').addClass('loading');
      ...
      else if($(e.target).parents(".file_attachment_div").find(".fa-flag").hasClass('flag_red'))
      {
        var flag_status = '0';
        $.ajax({
          url:"<?php echo URL::to('user/change_flag_status'); ?>",
          ...
          success:function(result)
          {
            $(e.target).parents(".file_attachment_div").find(".fa-flag").removeClass('flag_red');
            $(e.target).parents(".file_attachment_div").find(".fa-flag").addClass('flag_gray');
